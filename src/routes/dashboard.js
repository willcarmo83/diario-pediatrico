...
async function fetchEntries(childId, start, end) {
...
router.get("/diagnostico", async (req, res) => {
  ...
  const recent = await fetchEntries(childId, d7.toISOString(), now.toISOString());
  const previous = await fetchEntries(childId, d14.toISOString(), d7.toISOString());
